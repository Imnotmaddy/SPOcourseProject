...
public class AES128 {
    ...
    public void encryption(int[][] block, int[][] key) {
        ...
        subBytes(block);
        shiftRows(block);
        keyExpansion(key, Nr - 1);
        addRoundKey(block, key);
    }

    private void encryptBlock(int[][] block, int[][] key) {
        subBytes(block);
        shiftRows(block);
        mixColumns(block);
        addRoundKey(block, key);
    ...
    private void subBytes(int[][] block) {
        for (int i = 0; i < Nb; i++)
            subWord(block[i]);
    }

    private void subWord(int[] row) {
        for (int i = 0; i < Nb; i++)
            row[i] = subByte(row[i]);
    }
    ...
    private void rotateWordLeft(int[] row) {
        int temp = row[0];
    ...
    private void mixColumns(int[][] block) {
        for (int i = 0; i < Nb; i++) {
            ...
            for (int j = 0; j < Nb; j++)
                temp_col[j] = block[j][i];

            mixColumn(temp_col);

            for (int k = 0; k < Nb; k++)
                block[k][i] = temp_col[k];
        }
    }
    ...
    private void addRoundKey(int[][] block, int[][] key) {
        for (int i = 0; i < Nb; i++)
            for (int j = 0; j < Nb; j++)
                block[j][i] = block[j][i] ^ key[j][i];
    }

    private void keyExpansion(int[][] key, int roundCount) {
        int temp_row[] = new int[Nb];

        for (int i = 0; i < Nb; i++)
            temp_row[i] = key[i][Nb - 1];

        rotateWordLeft(temp_row);
        subWord(temp_row);

        for (int i = 0; i < Nb; i++)
            temp_row[i] = temp_row[i] ^ key[i][0] ^ Rcon[i][roundCount];
    ...
    private int galois_multx(int value) {
